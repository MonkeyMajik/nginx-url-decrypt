...
typedef struct {
  ...
  ngx_atomic_t                             lock;
#endif
  //add by yrc 2016 10 26
  ngx_uint_t                               check_hand; 
...
ngx_http_upstream_check_find_shm_peer(ngx_http_upstream_check_peers_shm_t *peers_shm,
				      ngx_addr_t *addr,ngx_str_t* name, ngx_pool_t *pool;);
				      ...
				      ngx_int_t result)
{
  ...
  if (result) {
    ...
    if (peer->shm->down && peer->shm->rise_count >= ucscf->rise_count&&peer->shm->check_hand == 0) {
      peer->shm->down = 0;
      ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0,
		    "enable check peer: %V ",
		    &peer->check_peer_addr->name);
...
ngx_http_upstream_check_status_handler(ngx_http_request_t *r)
{
  ...
  /* ngx_http_upstream_check_peer_t* peerTemp1;
     peerTemp1 = peers->peers.elts;
 
     ngx_uint_t n;
     for (n = 0; n < peers->peers.nelts; n++) {
     ngx_str_set(&peerTemp1[n].shm->group_name,(const char*) peerTemp1[n].upstream_name->data);
     peerTemp1[n].shm->group_name.len = peerTemp1[n].upstream_name->len;

     ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
     "group name %s",	peerTemp1[n].shm->group_name.data );
     }
  */

  //add by yrc 2016 10 26
  ngx_str_t arg_ip;
  ngx_str_set(&arg_ip,"token");
  ngx_str_t arg_hand;
  ngx_str_set(&arg_hand,"hand");
  ngx_str_t arg_group;
  ngx_str_set(&arg_group,"group");

  ngx_str_t value_ip;
  ngx_str_t value_hand;
  ngx_str_t value_group;
    
  if (ngx_http_arg(r, arg_ip.data, arg_ip.len, &value_ip)
      == NGX_OK&&ngx_http_arg(r, arg_hand.data, arg_hand.len, &value_hand)
      == NGX_OK&&ngx_http_arg(r, arg_group.data, arg_group.len, &value_group)
      == NGX_OK){

    ngx_http_upstream_check_peer_t* peerTemp;
    peerTemp = peers->peers.elts;
 
    ngx_uint_t i;
    for (i = 0; i < peers->peers.nelts; i++) {
        
      if(ngx_strncmp((const char*)value_ip.data,(const char*)peerTemp[i].peer_addr->name.data,peerTemp[i].peer_addr->name.len) == 0
	 &&value_ip.len == peerTemp[i].peer_addr->name.len
	 &&value_group.len ==  peerTemp[i].upstream_name->len
	 &&ngx_strncmp((const char*)value_group.data,(const char*) peerTemp[i].upstream_name->data, peerTemp[i].upstream_name->len) == 0){

 	ngx_shmtx_lock(&peerTemp[i].shm->mutex);

	if( 0 == ngx_atoi(value_hand.data,value_hand.len)){
	  peerTemp[i].shm->down = 0;
	  peerTemp[i].shm->check_hand = 0;
	}
	else if( 1 ==  ngx_atoi(value_hand.data,value_hand.len) ){
	  peerTemp[i].shm->down = 1;
	  peerTemp[i].shm->check_hand = 1;
	}
	ngx_shmtx_unlock(&peerTemp[i].shm->mutex);
      }
    }
  }
  //end of add 
...
ngx_http_upstream_check_init_shm_zone(ngx_shm_zone_t *shm_zone, void *data)
{
  ...
  for (i = 0; i < number; i++) {
    ...
    //modify by yrc 2016 11 03
    ngx_str_set(&peer_shm->group_name,(const char*)peer[i].upstream_name->data);
    peer_shm->group_name.len= peer[i].upstream_name->len;
    //end of modify
    ...
    if (opeers_shm) {
     ...
     opeer_shm = ngx_http_upstream_check_find_shm_peer(opeers_shm,
						      peer[i].peer_addr,peer[i].upstream_name, pool);
    if (opeer_shm) {
				      ...
				      ngx_addr_t *addr,ngx_str_t* name, ngx_pool_t *pool)
{
  ...
  for (i = 0; i < p->number; i++) {
    ...
    ngx_log_error(NGX_LOG_ERR, pool->log, 0, "group: %s",(const char*) peer_shm->group_name.data);
    ngx_log_error(NGX_LOG_ERR, pool->log, 0, "name: %s",(const char*) name->data);

    if (name->len !=  peer_shm->group_name.len){
      continue;
    }
    if (ngx_strcmp((const char*)name->data,(const char*) peer_shm->group_name.data) != 0){
      continue;
    }
    if (ngx_memcmp(addr->sockaddr, peer_shm->sockaddr, addr->socklen) == 0) {

      ngx_log_error(NGX_LOG_ERR, pool->log, 0,"find it !!!!!!!!!!!!!!!!!!!!!!!!");
      return peer_shm;
    }
  }
				      ...
				      ngx_pool_t *pool, ngx_str_t *name)
{
  ...
  if (opsh) {
    ...
    //modify by yrc 2016 11 02 

    char* a1 = inet_ntoa(((struct sockaddr_in*)opsh->sockaddr)->sin_addr);

    ngx_log_error(NGX_LOG_ERR, pool->log, 0, "addr: %s ",a1);

    ngx_log_error(NGX_LOG_ERR, pool->log, 0,
		  "rise: %d ",opsh->rise_count);
    ngx_log_error(NGX_LOG_ERR, pool->log, 0,
		  "fall: %d ",opsh->fall_count);
    ngx_log_error(NGX_LOG_ERR, pool->log, 0,
		  "hand: %d ",opsh->check_hand);
    ngx_log_error(NGX_LOG_ERR, pool->log, 0,
		  "down: %d ",opsh->down);
    ngx_log_error(NGX_LOG_ERR, pool->log, 0,
		  "#############################################");

    psh->check_hand   = opsh->check_hand;
    psh->down         = opsh->check_hand;
